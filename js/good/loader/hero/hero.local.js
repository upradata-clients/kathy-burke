...
const createHeroToImagePinAnimation = ({ heroBlock, signatureBlock, notreDameBlock }) => {
    ...
    const prepareLayoutForAnimation = () => {
        ...
        const createNotreDameImageInPainting = () => {
            const notreDameImgInPainting = /** @type {SVGSVGElement} */(_.queryThrow('svg', notreDameBlock).cloneNode(true));
        ...
        return {
            hero: {
                block: heroBlock, container: heroContainer,
                svg: { gallery: _.queryThrow('svg', heroBlock), centralPainting, notreDameImgInPainting }
            },
            signature: { block: signatureBlock, svg: _.queryThrow('svg', signatureBlock) },
            notreDame: {
                block: notreDameBlock, title: notreDameTitle, split: { ...titleSplit, groupsClone },
                svg: { hand: notreDameHand, sky: notreDameSky, notreDame: _.queryThrow('svg', notreDameBlock) }
            }
        };
    };

    const createAnimation = () => {
        const { hero, notreDame, signature } = prepareLayoutForAnimation();
        ...
        const createResizeObserver = () => {
            ...
            [ hero.svg.gallery, notreDame.svg.sky ].forEach(target => resizeObserver.observe(target));
        };
        ...
        const addToScrub = _.bindAddToTimeline({ timeline: tlScrub });
        const addToNotreDame = _.bindAddToTimeline({ timeline: notreDameTL });
        ...
        addToScrub(
            {
                target: hero.svg.notreDameImgInPainting,  // disappears while scrolling
                start: 0,
                set: {
                    position: 'absolute',
                    ...getLazyPositionElementTo({ to: hero.svg.centralPainting, type: 'xy' }),
                    ...getLazySize(hero.svg.centralPainting)
                }
            },
            {
                target: signature.block,  // disappears while scrolling
                start: 0,
            ...
            },
            {
                ...
                start: 0.2,
                to: { duration: 0.6, opacity: 1, ease: 'power2.inOut' }
            },
            {
                target: hero.svg.centralPainting, // disappears in the middle frame
                start: 0.2,
        ...
        addToNotreDame(
            {
                ...
                set: {
                    ...getLazyPositionElementTo({ to: hero.svg.centralPainting, type: 'top-left' }),
                    ...getLazySize(hero.svg.centralPainting),
                    autoAlpha: 1,
                }
            },
            ...
            {
                target: notreDame.split.container, start: '+=0.01', set: { opacity: 1, ...getLazyPositionElementTo({ to: notreDame.svg.sky }) },
            },

            {
                start: '+=0.5',
                timelines: {
                    elements: notreDame.split.all,
                        ...
                        return [
                            {
                                ...
                                from: {
                                    ...getLazyPositionElementTo({
                                        ...
                                        getRect: el => centerOfRect(getAbsoluteRect(el, { type: POSITION_TYPE.XY }))
                                    }),
                                    duration,
                                    stagger: 0.05
                                }
                            ...
                            {
                                target: notreDame.split.groupsClone[ i ], start: 0, fromTo: {
                                    from: { backgroundColor: 'rgba(255, 255, 255, 0)' },
                                    to: { duration, backgroundColor: 'rgba(255, 255, 255, 0.85)' }
                                }
                            },
                            {
                                target: notreDame.split.groupsClone[ i ], start: 0,
            ...
            {
                target: [ ...notreDame.split.groups, ...notreDame.split.groupsClone ], start: '+=0.2',
